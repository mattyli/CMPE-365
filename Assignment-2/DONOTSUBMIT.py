...
def buildTristrips( triangles ):
    ...
    unused_triangles = set(triangles) # makes a set of unused triangles for quick access
    used_tris = set()                 # contains a set of used triangles, O(1) lookup time
    num_used = 0                      # counts number of used triangles == len(used_tris)
    
    # sort the triangles by number of unused adjacent triangle (low to high)
    triangles.sort(key = lambda x: sum(i not in used_tris for i in x.adjTris))
    
    #start at the triangle with least adjacent triangles (higher chance they are corner triangles)
    triangle = triangles[0] 
    
    # adds the first triangle to used_tris
    used_tris.add(triangle) 
    
    # removes the first triangle from unused triangles
    unused_triangles.remove(triangle) 
    
    # counting the first strip
    count += 1 
    
    
    # while loop until we use all the triangles
    while(num_used != len(triangles)-1):
        
        # sort adjacent triangles by their number of adjacent triangles (low to high)
        triangle.adjTris.sort(key = lambda x: sum(i not in used_tris for i in x.adjTris))
        
        # starts by checking if the neighbouring triangle with the least unused adjacent triangles have been used
        if triangle.adjTris[0] not in used_tris:
            
            # moves to the next triangle in the strip
            triangle = moveToNextTri(triangle, 0)
            
            # adds that triangle to our used triangles
            used_tris.add(triangle)
            
            # removes the triangle from unused triangles
            unused_triangles.remove(triangle)
            
        # checks if there is more than 1 adjacent triangle and if it's unused
        elif len(triangle.adjTris) > 1 and triangle.adjTris[1] not in used_tris:
            triangle = moveToNextTri(triangle, 1)
            used_tris.add(triangle)
            unused_triangles.remove(triangle)
            
        # checks if there are more than 2 adjacent triangles and if it's unused
        elif len(triangle.adjTris) > 2 and triangle.adjTris[2] not in used_tris:
            triangle = moveToNextTri(triangle, 2)
            used_tris.add(triangle)
            unused_triangles.remove(triangle)
        
        # if all adjacent triangles are used
        else:
            
            # picks the unused triangle with the least number of used neighbours
            temp_tri_list = list(unused_triangles)
            temp_tri_list.sort(key = lambda x: sum(i not in used_tris for i in x.adjTris))
            triangle = temp_tri_list[0]
            
            # adds 1 to the count of TriStrips
            count += 1
            
            # adds triangle to used and removes from unused
            used_tris.add(triangle)
            unused_triangles.remove(triangle)
            
        # increments number of unsed triangles
        num_used+=1
...
# function to add a triangle to the strip, and "move" to that triangle
def moveToNextTri(old_triangle, index):
    
    # make the current triangle point to the new triangle
    old_triangle.nextTri = old_triangle.adjTris[index]
    
    # move to the new triangle
    new_triangle = old_triangle.nextTri
    
    # make the new triangle point back to the old triangle
    new_triangle.prevTri = old_triangle
