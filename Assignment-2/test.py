...
class Triangle(object):
    ...
    nextID = 0               # this variable is accessible by all instances of Triangle
    ...
    def __init__( self, verts ):
        ...
        self.nextTri = None  # next triangle on strip           POINTER
        self.prevTri = None  # previous triangle on strip       POINTER 
...
"""
    TRYING THIS WITHOUT RECURSION (ITERATIVE APPROACH)
"""
...
def buildTristrips(triangles: list[Triangle])-> None:
    ...
    # cast the input as a set for tracking what has and has not been used

    remainingTriangles = set(triangles)
    usedTris = set()

    triangles.sort(key = lambda x: sum(i not in usedTris for i in x.adjTris))
    
    startingTri = triangles[0]         # because after sorting, the first element of triangles has the least adj tris

    useTriangle(startingTri, remainingTriangles, usedTris)

    count = 1

    while remainingTriangles:
        triangles.sort(key = lambda x: sum(i not in usedTris for i in x.adjTris))
        startingTri = triangles[0]

        # starting from the first triangle, pick the next triangle (least adjacent triangles)
        nextTri = getNextTri(startingTri, usedTris)
        if not nextTri: # if all adjacent triangles are used
            useTriangle(startingTri, remainingTriangles, usedTris)
            goodToUse = list(remainingTriangles)
            goodToUse.sort(key = lambda x: sum(i not in usedTris for i in x.adjTris))
            startingTri = goodToUse[0]
            continue

        else:   # there are starting triangles
            startingTri.nextTri = nextTri
            nextTri.prevTri = startingTri
...
def getNextTri(tri: Triangle, usedTris: set)->Triangle:
    validAdjs = getValidAdjs(tri, usedTris)
    if not validAdjs: return
    else:
        nextTri = validAdjs[0]
    return nextTri

# something wrong
def getValidAdjs(tri: Triangle, usedTris: set)->list[Triangle]:
    result = []

    for adjTri in tri.adjTris:
        if adjTri not in usedTris:
            result.append(adjTri)
        
    result.sort(key=len)
    return result 


def useTriangle(aTriangle: Triangle, remainingTriangles: set, usedTris: set)-> None:
    remainingTriangles.remove(aTriangle)
    usedTris.add(aTriangle)
    return
